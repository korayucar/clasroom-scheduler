...
public class EventGeometry {
    ...
    public boolean computeEventRect(int date, int left, int top, int cellWidth, 
    		Event event, int index , int totalcals ) 
    {
        ...
        float singlewidth = cellWidth/totalcals;
        float little = singlewidth/event.intersects_with;
        Log.v("yhtr",""+event.intersects_with);
       
        
        event.left =left+singlewidth*index+little*event.intersect_index;// left + col * (colWidth + mCellMargin);
        event.right =left+singlewidth*index+little*(1+event.intersect_index);// event.left + colWidth;
        return true;
    }
